    ...
    ui(new Ui::MainView)
{
    ...
    connect(sc,SIGNAL(alarm(int,QString,int)),this,SLOT(getAlarm(int,QString,int)));
    sound = new QSound("D:/Qt/projects/Server/sound/alarm.wav");
    sound->setLoops(-1);
    ui->setupUi(this);
...
void MainView::getAlarm(int ID, QString IP, int position)
{
    qDebug()<<"here";
    QMessageBox msgBox;
    QString name;
    ...
    if(position ==1)
    {
        QSqlQuery query = DataBase::DBQuery("select*from doctor where DId ='"+QString("%1").arg(ID)+"';");
        query.next();
        name = query.value(2).toString();
        msgBox.setText(QString("接到报警，来自医生:%1(ip：%2)").arg(name,IP));
        msgBox.exec();
    }
    else if(position ==2)
    {
        QSqlQuery query = DataBase::DBQuery("select*from patient where PId ='"+QString("%1").arg(ID)+"';");
        query.next();
        name = query.value(2).toString();
        msgBox.setText(QString("接到报警，来自患者:%1(ip：%2)").arg(name,IP));
        msgBox.exec();
    }
    sound->stop();
}
...
void MainView::setExaminationModel()
{
    ...
    model->setHeaderData(2,Qt::Horizontal,"指标编号");
    model->setHeaderData(3,Qt::Horizontal,"数值");
...
void MainView::setTreatmentModel()
{
    ...
    model->setHeaderData(5,Qt::Horizontal,"建议");
    model->setHeaderData(6,Qt::Horizontal,"处方");
